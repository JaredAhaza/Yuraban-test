    ...
    <div class="py-6">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div class="flex justify-between items-center mb-6">
                <h1 class="text-2xl font-bold text-gray-900">Drivers Awaiting Approval</h1>
                
                <!-- Back to Drivers button -->
                <a href="{{ route('drivers.index') }}" class="inline-flex items-center px-4 py-2 bg-gray-100 border border-gray-300 rounded-md font-semibold text-xs text-gray-700 uppercase tracking-widest shadow-sm hover:bg-gray-200 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:ring-offset-2 transition ease-in-out duration-150">
            ...
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                @if(count($drivers) === 0)
                    <div class="p-12 text-center text-gray-500">
                        ...
                        <h3 class="text-lg font-medium">No drivers awaiting approval</h3>
                        <p class="mt-2">All driver applications have been processed.</p>
                    </div>
                @else
                                    ...
                                    <tr class="hover:bg-gray-50">
                                        ...
                                        <td class="px-6 py-4 whitespace-nowrap">
                                            <div class="text-sm text-gray-900">
                                                @if(isset($driver->county) && $driver->county)
                                                    {{ $driver->county->name ?? $driver->county->county_name ?? 'County #'.$driver->county_id }}
                                                @else
                                                    --
                                                @endif
                                            </div>
                                            <div class="text-sm text-gray-500">{{ $driver->subcounty ?? '--' }}</div>
                                        </td>
                                        ...
                                        <td class="px-6 py-4 whitespace-nowrap text-sm font-medium">
                                            <div class="flex space-x-2">
                                                ...
                                                <form action="{{ route('admin.drivers.decline', $driver->id) }}" method="POST">
                                                    @csrf
